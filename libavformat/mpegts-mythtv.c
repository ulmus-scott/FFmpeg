...
int ff_mythtv_parse_mpeg2_descriptor(AVFormatContext *fc, AVStream *st, int stream_type, int prg_id,
...
static void pmt_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    for (i = 0; i < MAX_STREAMS_PER_PROGRAM; i++) {
        ...
        if (desc_list_end > p_end)
            goto out;
        for (;;) {
            if (ff_mythtv_parse_mpeg2_descriptor(ts->stream, st, stream_type, h->id, &p,
